...
const setupAnimation = () => {
  ...
  const animate = () => {
    ...
    const relaxation = props.relaxation ?? 0.9;
    const strength = props.strength ?? 0.15;
    const mouseInfluence = props.mouse ?? 0.1;

    for (let i = 0; i < size * size; i++) {
      const idx = i * 4;
      data[idx] = (data[idx] ?? 0) * relaxation;
      data[idx + 1] = (data[idx + 1] ?? 0) * relaxation;
    }

    const gridMouseX = size * mouseState.x;
    const gridMouseY = size * mouseState.y;
    const maxDist = size * mouseInfluence;
    ...
    for (let i = 0; i < size; i++) {
      for (let j = 0; j < size; j++) {
        ...
        if (distSq < maxDist * maxDist) {
          ...
          data[index] = (data[index] ?? 0) + strength * 100 * mouseState.vX * power;
          data[index + 1] = (data[index + 1] ?? 0) - strength * 100 * mouseState.vY * power;
        }
      }
    }
...
<template>
  <div ref="containerRef" :class="[props.className, 'grid-distortion']" />
</template>
