...
const prepStops = (stops?: string[]) => {
  let base = (stops && stops.length ? stops : ['#FF9FFC', '#27FF64']).slice(0, MAX_COLORS) as string[];
...
const setupGyroscope = (canvas: HTMLCanvasElement, renderer: Renderer): (() => void) | null => {
  ...
  const onTilt = (e: DeviceOrientationEvent) => {
    ...
    const beta  = Math.max(-45, Math.min(45, (e.beta ?? 0) - 20)); 

    const x = ((gamma + 45) / 90) * rect.width  * scale;
    const y = ((beta  + 45) / 90) * rect.height * scale;

    mouseTargetRef.value = [x, y];
  };

  const register = () => window.addEventListener('deviceorientation', onTilt);
...
`;
  ...
  // ← Gyroscope registered right after the pointer listener
  const gyroCleanup = setupGyroscope(canvas, renderer);
